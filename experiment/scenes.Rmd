...
Get the ranges of the values of P(A|C) and P(C|A) when varying the range of P(A) and P(C). 
The constraints how P(A) and P(C) vary are:
* low: p <= x0
* uncertain: x1 <= p <= x2
* high: p >= x3
where x0 < x1 < x2 < x3.
...
data <- crossing(pa = constraints, pc = constraints) %>%
  mutate(pac=list(seq(0, 1, 0.005))) %>% unnest() %>% 
...
```{r}
data <- data %>% mutate(ca.min =
                          case_when(ca.min.val <= low ~ "low",
...
df <- df %>% unnest(ac.vals, .drop = FALSE) %>% select(-ac.max, -ac.min, -ca.max, -ca.min)

combinations <- df %>% ungroup() %>% distinct_at(vars(-scene_id), .keep_all = TRUE)
combinations
```
...
```{r}
combinations <- combinations %>% rename(pc_given_a=ca.vals, pa_given_c=ac.vals)

combis_luh <- combinations %>%
  filter_at(vars(-scene_id), all_vars(. == "uncertain" | . == "high" | . == "low"))
combis_luh

combis_lh <- combinations %>% filter_at(vars(-scene_id), all_vars(. == "high" | . == "low"))
combis_lh
```
...
```{r}
df <- combis_luh %>% rowid_to_column()
df_dep <- df %>% replace(.=="low", 1) %>% replace(.=="uncertain", 2) %>% replace(.=="high", 3) %>% 
...
df <- left_join(df, df_dep, by="rowid")
df
```
