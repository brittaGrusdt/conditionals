...
Sanity check.
After annotation, check for duplicates in scene setting.

```{r}
annotations <- read_csv("./stimuli/scenes_luh_annotations.csv")
annotations

nrows <- nrow(annotations)
nrows

setup_vars <- annotations %>% select("id", "pa", "pc", "A.orientation",	"C.orientation", "platform.type",	"platform1.height",	"platform1.width", "platform2.height", "platform2.width", "platform.dist")
