...
df <- df %>% mutate(idx = case_when(order == "low" ~ 1,
                   ...
                   ) %>% group_by(scene_id) %>% select(-order) %>% spread(key=vals, val=idx)
...
combinations <- df %>% ungroup() %>% select(-scene_id) %>% distinct()
combinations
```
...
combis_luh <- combinations %>%
  filter_all(all_vars(. == "uncertain" | . == "high" | . == "low"))
combis_luh

combis_lh <- combinations %>% filter_all(all_vars(. == "high" | . == "low"))
combis_lh
```
