...
var make_utterances = function(var_powerset){
  ...
  var conditionals_all = conditionals.concat(conditionals_likely)
...
var meaning = cache(function(utterance, table){
 ...
 var u_applicable = utterance=="" ? true :
                    (utterance.includes('likely') ? p >= thresholds.theta_likely : p >= thresholds.theta)
                      // (p >= thresholds.theta_likely) :
                      // (utterance.includes('-(') ? p <= 1-thresholds.theta : p >= thresholds.theta)

                    // utterance.includes(">") ? ((p >= degree - 0.025) &
                    //(p <= degree + 0.025)) :  p >= thresholds.theta
 return u_applicable
})
