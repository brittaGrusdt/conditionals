...
plot_marginal_prob(pc, c("C"), evs=evs_all %>% filter(p=="C"), save_as=file.path(RESULT_DIR, "figs", "none-pc.png"))
plot_marginal_prob(data_wide, "cns", save_as=file.path(RESULT_DIR, "figs", "none-cns.png"))
...
plot_marginal_prob(pc, c("C"), evs=filter(evs, p=="C"), save_as=file.path(RESULT_DIR, "figs", "pizza-pc.png"))
```
...
# plot_marginal_prob(pa, c("A"), evs=expected_val(pa, "A"), save_as=file.path(RESULT_DIR, "figs", "lawn-pa.png"))
# plot_marginal_prob(pc, c("C"), evs=expected_val(pc, "C"), save_as=file.path(RESULT_DIR, "figs", "lawn-pc.png"))

plot_marginal_prob(data_wide, "cns", save_as=file.path(RESULT_DIR, "figs", "lawn-cns.png"))
```
...
```{r}
voi_none <- voi_none_all %>% filter(key=="epistemic_uncertainty_C") %>% filter_tables(params) %>%
...
ggsave(paste(RESULT_DIR, "figs", "none-voi-epistemic-uncertainty.png", sep=.Platform$file.sep), p)
p
```
...
ggsave(paste(RESULT_DIR, "figs", "pizza-voi-pc.png", sep=.Platform$file.sep), p)
p
```
...
ggsave(paste(RESULT_DIR, "figs", "none-vois-cp.png", sep=.Platform$file.sep), p)
p

# plot only values for bias=lawn
p <- plot_cp_vois(data %>% filter(bias=="lawn"))
ggsave(paste(RESULT_DIR, "figs", "lawn-vois-cp.png", sep=.Platform$file.sep), p)
p

# plot comparison between cp-strengths for bias-none and bias-lawn
p <- plot_cp_vois(data)
ggsave(paste(RESULT_DIR, "figs", "comparison-vois-cp.png", sep=.Platform$file.sep), p)
p
```
...
<!-- plot_voi_alpha_cost(data, 1, "epistemic_uncertainty_C", "PL") -->
